...
def cursor(screen):
    ...
    cursor_image = pygame.image.load('cursor/cursor.jpg')
    cursor_rect = cursor_image.get_rect()
    if pygame.mouse.get_focused():
        cursor_rect.center = pygame.mouse.get_pos()  # Получаем позицию
...
def main_menu(screen):
    ...
    while running:  # Главный цикл меню
        ...
        for event in pygame.event.get():  # Обрабатываем события
            ...
            # if pygame.mouse.get_focused():    # Проверяем, находится ли мышь внутри окна
            #     cursor_rect.center = pygame.mouse.get_pos()  # Получаем позицию курсора
            #     pygame.mouse.set_visible(False)
            #     draw()
            #     screen.blit(cursor_image, cursor_rect)
            # else:
